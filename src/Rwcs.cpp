...
struct wcsprm* _read_from_header(int *nwcs, struct wcsprm** wcs, Rcpp::String header, int nkeyrec, int WCSref)
{
  int nreject;
                  ...
                  int nkeyrec, int WCSref=0)
{
  ...
  int nwcs;
  struct wcsprm* wcs;
  auto wcs_at_ref = _read_from_header(&nwcs, &wcs, header, nkeyrec, WCSref);
  if (!wcs_at_ref) {
    wcsvfree(&nwcs, &wcs);
    return nullptr;
  }
